    ...
    def forward(self, features, adj):
        ...
        if not isinstance(adj, torch.sparse.FloatTensor):
            if len(adj.shape) == 3:
                ...
                # neigh_feature = torch.spmm(adj.float(), features) / (adj.to_dense().sum(dim=1).reshape(adj.shape[0], -1) + 1)
                # neigh_feature = torch.mm(adj, features) / (adj.sum(dim=1).reshape(adj.shape[0], -1) + 1)
                # neigh_feature = torch.spmm(adj, features) / (
                #             torch.sparse.sum(adj, [0]).values().reshape(adj.shape[0], -1) + 1)
                neigh_feature = torch.spmm(adj, features) / (
                            torch.sparse.sum(adj, [0]).values().reshape(adj.shape[0], -1) + 1)
        else:
            ...
            neigh_feature = torch.spmm(adj, features) /(torch.sparse.sum(adj,[1]).values().reshape(adj.shape[0],-1)+1)
